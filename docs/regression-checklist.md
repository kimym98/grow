...
- [ ] (Task 015) `job_postings`/`tech_news`에 신규 레코드가 수집되면(pg_cron 실행 또는 수동 INSERT) 실행 중인 앱에 "새 공고 N건"/"새 뉴스 N건" OS 알림이 표시된다. 여러 건이 짧은 시간 안에 연속 삽입돼도 알림은 한 번만(3초 디바운스) 뜬다.
- [ ] (Task 015) 앱 시작 시 electron-updater가 업데이트를 확인한다(개발 모드에서는 "Skip checkForUpdates because application is not packed" 로그가 정상). 실제 배포 빌드에서 신규 버전 발견 시 다운로드 완료 알림이 뜨고 재시작하면 적용된다 — **macOS는 코드 서명이 없으면 자동 업데이트 자체가 동작하지 않으며(electron-builder 공식 제약), Windows는 `verifyUpdateCodeSignature: false`로 서명 없이도 설치되도록 설정했다.** 서명된 배포 빌드/실제 GitHub Release가 없는 현재 환경에서는 다운로드~설치까지의 end-to-end 검증은 하지 못했다.
- [ ] (Task 015) 설정 페이지에서 "OS 로그인 시 자동 실행"을 켜면 실제 OS 시작 프로그램(Windows는 `Win+R` → `shell:startup` 또는 작업 관리자의 시작프로그램 탭)에 앱이 등록되고, 끄면 해제된다.
- [ ] (Task 015) "창을 닫아도 백그라운드 유지"를 켠 상태에서 창을 닫아도 프로세스가 종료되지 않고 트레이 아이콘이 남으며, 트레이 아이콘 클릭 또는 "열기" 메뉴로 창을 다시 열 수 있고 "종료" 메뉴로 완전히 종료된다. 옵션이 꺼져 있으면(기본값) 기존과 동일하게 창을 닫으면 앱이 종료된다(비-macOS 기준).
- [ ] (Task 015) **앱 프로세스가 완전히 종료된 상태(트레이 유지 미사용 시 창을 닫은 경우, 또는 트레이 메뉴로 종료한 경우)에서는 일정 알림/Realtime 수집 알림 모두 발송되지 않는다** — 메인 프로세스가 렌더러의 IPC push에 의존하는 캐시 구조이기 때문이며, 알림이 보장되려면 트레이 유지 옵션을 켜서 프로세스를 상시 구동시켜야 한다.
...
- (Task 015) "OS 로그인 시 자동 실행"/"창을 닫아도 백그라운드 유지" 토글도 같은 이유로 실제 설정 화면 클릭 E2E는 자동화하지 못했다. 대신 `app.setLoginItemSettings`/`Tray` 생성 API 자체가 이 개발 환경(Windows)에서 정상 동작함(등록 시 Windows 시작 항목에 실제로 추가/해제됨, Tray 인스턴스 생성/파괴가 에러 없이 동작함)은 Electron 헤드리스 스크립트로 별도 실측했다.
