...
- **크롤링 소스 확장(일부 재개) + 기업 분석**: 캐치·자소설닷컴·사람인·인크루트는 ToS(크롤링 금지 조항)로 최종 제외, 커리어(career.co.kr)·링커리어(linkareer.com)는 잡코리아와 동등한 리스크 수준으로 판단해 착수 확정(Task 033·034), 자체 수집 공고 기반 LLM 기업 분석 유지
...
- **Task 030: 크롤링 소스 확장 및 DOM 파서 교체** - ❌ 취소 (2026-09-01, Task 029에서 캐치·자소설닷컴 모두 ToS 위반으로 최종 제외 확정)
  ...
  - 신규 소스가 필요할 경우 부록의 원티드/사람인 공식 API(인증키 신청 필요) 경로를 후속 조사 대상으로 재검토 — ⚠️ 2026-09-01 Task 032 조사 결과 **사람인은 API 여부와 무관하게 이용약관(기업회원)이 크롤링을 명시적으로 금지**하므로 API 경로도 재검토 필요(신청 승인 시 API ToS를 별도 확인해야 함)

- **Task 032: 링커리어·사람인·인크루트·커리어 크롤링 가능 여부 조사** - ✅ 완료 (2026-09-01)
  - PRD 참조: 2.9 / 우선순위: 낮음(정보 수집용, 실제 소스 추가는 별도 Task) / 선행 조건: 없음
  - robots.txt(curl)와 이용약관(Chrome DevTools MCP 실제 렌더링)을 각각 확인. 상세 근거는 `docs/research/job-source-research.md`의 "Task 032" 절 참고
  ...
  - **후속 필요**: 링커리어·커리어를 실제로 소스로 추가할지는 이번 Task 범위 밖(가능 여부 판정까지만 수행) — 커리어는 Task 033, 링커리어는 Task 034로 착수 확정(2026-09-01 사용자 승인)

- **Task 033: 커리어(career.co.kr) 크롤링 소스 추가** - ✅ 완료 (2026-09-01)
  - PRD 참조: 2.9 / 우선순위: 중 / 선행 조건: **Task 032 (착수 가능 판정 완료)**, Task 024
  - Task 032 조사 결과 커리어는 robots.txt가 채용정보 경로(`/signup`, `/login`, `/resume`, `/user`, `/company`만 차단, 나머지 허용)를 막지 않고, 이용약관(2018-04-13 시행)에도 크롤링을 콕 집어 금지하는 조항이 없어(일반 재배포 금지 조항만 존재) 잡코리아와 동일한 리스크 수준으로 판단해 착수 확정. 사용자가 개인 사이드 프로젝트·비상업·저빈도 수집이라는 점을 감안해 이 회색지대 리스크를 인지하고 진행 승인함(2026-09-01)
  - **실측 결과 도메인 정정**: Task 032 조사 시점의 robots.txt는 `www.career.co.kr` 기준이었으나, 실제 채용정보 목록/상세는 서브도메인 **`job.career.co.kr`**에서 서빙됨을 구현 착수 전 curl 실측으로 확인. `job.career.co.kr/robots.txt`도 별도 재확인(`/admin`,`/app`,`/base`,`/biz`,`/user`,`/signup`만 차단, 목록 경로 허용)
...
- **Task 034: 링커리어(linkareer.com) 크롤링 소스 추가**
  - PRD 참조: 2.9 / 우선순위: 중 / 선행 조건: **Task 032 (착수 가능 판정 완료)**, Task 024
  - Task 032 조사 결과 링커리어는 robots.txt가 채용 경로를 허용(교육 콘텐츠 `/stem/learn/*` 경로만 차단)하고, 이용약관의 명시적 크롤링/스크래핑 금지 조항(제20조)은 링커리어 STEM(교육) 서비스의 기출콘텐츠에만 한정되어 채용정보에는 적용되지 않음. 다만 제39조(회원의 금지행위) 2호에 "자동화 프로그램·매크로 등 기계적 수단을 이용하여 서비스에 접근"하는 행위를 금지하는 다소 모호한 조항이 있어(게시판/채팅 도배 방지 맥락으로 보이나 문언상 완전히 배제되지는 않음) 커리어보다 근소하게 더 회색지대. 사용자가 개인 사이드 프로젝트·비상업·저빈도 수집이라는 점을 감안해 이 리스크를 인지하고 진행 승인함(2026-09-01)
  - `sources/linkareer.ts` 신규 어댑터 구현 — 잡코리아 어댑터(`sources/jobkorea.ts`)와 동일한 `JobPostingSource` 인터페이스 준수
  - **구현 착수 전 실측 필요(raw HTML 구조 미확인)**: 채용 목록 페이지(`https://linkareer.com/recruit-home` 등)를 curl/Chrome DevTools MCP로 fetch해 다음을 확정해야 함
    - 목록 페이지 URL 및 실제 파라미터(직무/지역/경력 필터 등)
    - 서버 렌더링(SSR) 여부 — 링커리어는 React 기반 SPA일 가능성이 있어(미확인) 클라이언트 사이드 렌더링이면 GraphQL/REST API 엔드포인트를 별도로 특정해야 함(캐치 사례처럼 JS 번들 정적 분석 필요할 수 있음)
    - 공고 행 단위 구조, 제목/회사/지역/경력/마감일 필드의 정확한 매핑
    - 페이지네이션 방식
  - `job_postings.source`에 `linkareer` 값 추가(스키마 필드 추가 없음), 기존 UI 필터/구분 로직 재사용
  - 기존 소스별 실패 격리(try/catch) 및 수집 로그 테이블 기록 패턴 유지, Task 024의 딜레이/모니터링 인프라 재사용
  - User-Agent는 잡코리아·커리어와 동일하게 중립적이고 식별 가능한 문자열 사용(예: `grow-job-collector/1.0`), robots.txt가 허용한 경로만 접근하고 `/stem/learn/*` 등 교육 콘텐츠 경로는 절대 접근하지 않음(제20조 크롤링 금지 조항이 명시적으로 적용되는 영역)
  - 완료 기준
    - [ ] 채용정보 목록 페이지 raw HTML(또는 API) 구조가 실측되고 SSR/CSR 여부가 확인됨(CSR로 API 특정이 어려우면 이 Task는 보류로 전환)
...
- **Task 031: 기업 분석 기능 구현 (`analyze-company`)** - 우선순위
  - PRD 참조: 2.10 / 우선순위: 중 / 선행 조건: **Task 019(마이그레이션), Task 025(LLM 통합), Task 027(해시 캐시)**, (선택) **Task 033(커리어 소스 추가), Task 034(링커리어 소스 추가)** — Task 030(캐치/자소설닷컴)은 취소되었으나 Task 033·034(커리어·링커리어)가 착수 확정되어, 완료 시 잡코리아+커리어+링커리어 최대 3개 소스 데이터로 기업 분석 범위를 잡을 수 있음(Task 033·034 없이 잡코리아 단독으로 먼저 진행해도 무방)
