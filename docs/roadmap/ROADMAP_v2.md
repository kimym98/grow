...
| Phase 1 | 1단계 | 상 | 020~023 | 보안 이슈 해소 + Sentry 실연동 (2.3, T-L3, 2.4, 2.18, 2.8 신청) |
| Phase 2 | 2단계 | 상 | 024~028 | 스크레이핑 견고성·아키텍처·성능 (2.7 선행분, 2.5, 2.6, 2.12, 2.13) |
...
### Phase 1: 보안 이슈 해소 및 모니터링 실연동 (1단계)

- **Task 020: CSP 최소 적용 및 렌더러 심층방어** - 우선순위
  - PRD 참조: 2.15 T-L3 (우선순위 상향) / 우선순위: 상 / 선행 조건: Task 018
...
- **Task 021: LLM API 키 검증 경로의 서버 이전 (`validate-llm-key`)**
  - PRD 참조: 2.3 / 우선순위: 상 / 선행 조건: **Task 020 (CSP)**
...
- **Task 022: 테스트 계정 자격증명 프로덕션 노출 방지**
  - PRD 참조: 2.4 / 우선순위: 상 / 선행 조건: Task 018
...
- **Task 023: Sentry DSN 실연동 및 대시보드 구축**
  - PRD 참조: 2.18 / 우선순위: 상 / 선행 조건: **Task 018** (동적 import 전환 완료 필요)
...
- **Task 023-1: 코드 서명 인증서 발급 신청 (병행 착수)**
  - PRD 참조: 2.8 / 우선순위: 중(리드타임 확보 목적으로 1단계 병행) / 선행 조건: 없음
...
- **Task 024: 크롤링 요청 딜레이·백오프 및 실패율 모니터링** - 우선순위
  ...
  - 완료 기준
    - [ ] 짧은 시간 반복 호출 시 대상 사이트로부터 429/403 없이 정상 수집
    - [ ] 실패율 임계치 초과를 인위 재현했을 때 알림이 실제 발송됨
    - [ ] 소스 확장(Task 030) 시 그대로 재사용 가능한 공통 유틸로 분리됨
...
- **Task 035: 코드 서명 적용 및 업데이트 검증 활성화** - 우선순위
  - PRD 참조: 2.8 / 우선순위: 중 / 선행 조건: **Task 023-1 (인증서 확보)**
