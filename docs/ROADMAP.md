...
- **Task 003: 공통 타입·스키마 정의 및 데이터베이스 스키마 설계**
  ...
  - Supabase 테이블 스키마 설계 문서화(컬럼, 인덱스, upsert 고유키, RLS 정책 초안) — 실제 마이그레이션 실행은 Phase 3
...
- **Task 008: Supabase 스키마 마이그레이션 및 인증/권한 구현** - 우선순위
  ...
  - Supabase Auth 연동(이메일/OAuth), Electron 딥링크 기반 로그인 콜백 처리
  - 세션 저장/복원, 보호 라우트 가드, 로그아웃 처리
  - Playwright MCP로 로그인/로그아웃/보호 라우트 접근 차단 E2E 테스트

- **Task 009: 채용 공고 수집 Edge Function 및 목록 연동**
...
- **Task 010: 일정관리 CRUD 및 Electron 네이티브 알림 구현**
...
- **Task 011: IT 뉴스 수집 Edge Function 및 피드 연동**
...
- **Task 012: PDF 첨삭 파이프라인 및 LLM Provider 구현**
...
- **Task 013: CS 면접 퀴즈 문제 뱅크 및 풀이 로직 구현**
